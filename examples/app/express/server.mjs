#!/usr/bin/env node
/**
 * Normal app example — HTTP server using @mbsks/side-pigeon.
 *
...
// Published: `bun add @mbsks/side-pigeon` then `from "@mbsks/side-pigeon"`.
...
await pc.start();
console.log(
  `[app] side-pigeon started providers=${providers.map((p) => p.id)} rss=${(process.memoryUsage().rss / 1048576).toFixed(1)}MB`,
);
...
server.listen(PORT, () =>
  console.log(`[app] listening http://localhost:${PORT}  POST /send  GET /events  GET /health`),
);
