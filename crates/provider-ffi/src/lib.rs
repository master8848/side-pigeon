                ...
                if let Some(arr) = v.get("providers").and_then(|x| x.as_array()) {
                    ...
                    for item in arr {
                        if let Some(id) = item.get("id").and_then(|x| x.as_str()) {
                            providers.push(ProviderConfigFallback {
                                id: id.to_string(),
                                config: item
                                    .get("config")
                                    .cloned()
            ...
            if let Ok(extra) = std::env::var(format!("PC_{upper}_CONFIG")) {
                if !extra.trim().is_empty() {
                    if let Ok(extra_v) = serde_json::from_str::<serde_json::Value>(&extra) {
                        if let (Some(base), Some(extra_o)) =
                            (config.as_object_mut(), extra_v.as_object())
                        {
                            for (k, v) in extra_o {
...
fn config_str_alt(config: &serde_json::Value, keys: &[&str]) -> Option<String> {
    for k in keys {
        if let Some(v) = config_str_opt(config, k) {
            return Some(v);
        }
    }
    None
...
fn build_provider(
    id: &str,
    config: &serde_json::Value,
    events: Arc<dyn provider_core::ProviderEvents>,
) -> Result<Box<dyn provider_core::ChatProvider>, String> {
    ...
    match kind {
        "demo" => {
            #[cfg(feature = "demo")]
            {
                Ok(Box::new(provider_core::alias::AliasedProvider::new(
                    alias_static,
                    Box::new(demo_provider::DemoProvider::new(events, config)),
                )))
            }
        ...
        "telegram" => {
            #[cfg(feature = "telegram")]
            {
                ...
                if let Some(base) = config_str_alt(config, &["base_url", "baseUrl"]) {
                    p = p.with_base_url(base);
                }
                if let Some(v) = config_u64_opt(config, "poll_interval_secs")? {
                    p = p.with_poll_interval(std::time::Duration::from_secs(v));
                }
                if let Some(v) = config_u64_opt(config, "long_poll_timeout_secs")? {
                    p = p.with_long_poll_timeout_secs(v);
                }
                if let Some(v) = config_u64_opt(config, "request_timeout_secs")? {
                    p = p.with_request_timeout(std::time::Duration::from_secs(v));
                }
                Ok(Box::new(provider_core::alias::AliasedProvider::new(
                    alias_static,
                    Box::new(p),
        ...
        "discord" => {
            #[cfg(feature = "discord")]
            {
                ...
                if let Some(url) = config_str_alt(config, &["gateway_url", "gatewayUrl"]) {
                    p = p.with_gateway_url(url);
                }
                if let Some(base) = config_str_alt(config, &["rest_base", "restBase"]) {
                    p = p.with_rest_base(base);
                }
                if let Some(v) = config_u64_opt(config, "intents")? {
                    p = p.with_intents(v);
                }
                if let Some(v) = config_u64_opt(config, "request_timeout_secs")? {
                    p = p.with_request_timeout(std::time::Duration::from_secs(v));
...
fn persist_path_from_cfg(cfg_json: Option<&str>) -> Option<String> {
    ...
    if let Ok(p) = std::env::var("PC_PERSIST_PATH") {
        if !p.trim().is_empty() {
            return Some(p);
        }
    }
...
impl PcHandle {
    fn new(cfg_json: Option<&str>) -> Result<Self, String> {
        ...
        for pc in &cfg.providers {
            match build_provider(&pc.id, &pc.config, events.clone()) {
                Ok(b) => {
                    builder = builder.provider(b);
                }
                Err(e) => {
                    failures.push(format!("{}: {e}", pc.id));
                    tracing::warn!("pc_init: skip provider {}: {e}", pc.id);
                }
            }
        }
        ...
        let mut client = match builder.build() {
            Ok(c) => Some(c),
            Err(e) => {
                tracing::warn!("pc_init: builder.build failed: {e}");
                None
            }
        };
        ...
        let persist_log = {
            ...
            if let Some(path) = path_opt {
                match provider_transport::persist::EventLog::open(&path) {
                    Ok(log) => {
                        tracing::info!(path=%path, "pc_init: persist log opened");
                        Mutex::new(Some(log))
                    }
                    Err(e) => {
                        tracing::warn!(path=%path, error=%e, "pc_init: persist open failed");
                        Mutex::new(None)
                    }
                }
            } else {
    ...
    pub fn send_text(&self, provider: &str, chat: &str, text: &str) -> Result<String, String> {
        ...
        let res = self
            .rt
            .block_on(async { client.send(provider, msg).await })
...
#[allow(dead_code)]
#[cfg(feature = "demo")]
fn builder_is_empty(b: &provider_core::client::ProviderClientBuilder) -> bool {
    let _ = b;
    true
}
#[allow(dead_code)]
#[cfg(not(feature = "demo"))]
fn builder_is_empty(_b: &provider_core::client::ProviderClientBuilder) -> bool {
    true
}
...
fn parse_event_filter(s: &str) -> Result<EventFilter, String> {
    ...
    let channel_id = v
        .get("channel_id")
        .or_else(|| v.get("channelId"))
        .or_else(|| v.get("room"))
        .and_then(|x| x.as_str())
        .map(|s| s.to_string());
    let explicitly_addressed = v
        .get("explicitly_addressed")
        .or_else(|| v.get("explicitlyAddressed"))
...
pub unsafe extern "C" fn pc_send(
    ...
    let Some(prov) = (unsafe { cstr_to_str(provider) }) else {
        return -1;
    };
    let Some(ch) = (unsafe { cstr_to_str(chat) }) else {
        return -1;
    };
    let Some(tx) = (unsafe { cstr_to_str(text) }) else {
        return -1;
    };
    let h = unsafe { &*handle };
...
pub unsafe extern "C" fn pc_subscribe(handle: *mut PcHandle, filter_json: *const c_char) -> i32 {
    ...
    let Some(s) = (unsafe { cstr_to_str(filter_json) }) else {
        return -1;
    };
    if s.trim().is_empty() {
        return 0;
    }
    let filter = match parse_event_filter(s) {
        Ok(f) => f,
        Err(e) => {
    ...
    match h.subscribe_filtered(filter) {
        Ok(()) => {
            tracing::debug!("pc_subscribe: filtered subscription added");
            0
        }
        Err(e) => {
            tracing::warn!("pc_subscribe failed: {e}");
            -1
        }
    }
}
