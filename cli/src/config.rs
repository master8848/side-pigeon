//! CLI configuration: JSON file or environment variables.
//!
//! NOTE: this module is copied verbatim from `bin/pc/src/config.rs`
//! (provider-connect sidecar) so pc-connect speaks the EXACT same config
//! contract (`PC_PROVIDERS`, `PC_<ID>_TOKEN`, `PC_<ID>_CONFIG`, `PC_CONFIG`,
//! `--config`). The sidecar remains the single source of truth; keep the two
//! in sync when the env contract changes.
